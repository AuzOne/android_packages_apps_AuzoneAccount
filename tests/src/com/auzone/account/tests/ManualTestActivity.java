...
package com.auzone.account.tests;
...
public class ManualTestActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.auzoneaccount_test);
        mSpinner = (Spinner) findViewById(R.id.accounts);
        mAccountManager = AccountManager.get(this);
        final Account[] accounts = mAccountManager.getAccountsByType(AuzoneAccount.ACCOUNT_TYPE_AuzoneAccount);
    ...
    private void getToken() {
        mAccountManager.getAuthToken(mSelectedAccount, AuzoneAccount.AUTHTOKEN_TYPE_ACCESS, null, this, new AccountManagerCallback<Bundle>() {
            @Override
    ...
    private void invalidateAuthToken() {
        String token = mAccountManager.peekAuthToken(mSelectedAccount, AuzoneAccount.AUTHTOKEN_TYPE_ACCESS);
        if (token != null) {
            mAccountManager.invalidateAuthToken(AuzoneAccount.AUTHTOKEN_TYPE_ACCESS, token);
    ...
    private void expireAuthToken() {
        mAccountManager.setUserData(mSelectedAccount, AuzoneAccount.AUTHTOKEN_EXPIRES_IN, "0");
        String token = mAccountManager.peekAuthToken(mSelectedAccount, AuzoneAccount.AUTHTOKEN_TYPE_ACCESS);
        if (token != null) {
            mAccountManager.invalidateAuthToken(AuzoneAccount.AUTHTOKEN_TYPE_ACCESS, token);
        }
    ...
    private void enableSetup() {
        ...
        ComponentName componentName = new ComponentName("com.auzone.account", "com.auzone.account.ui.SetupWizardActivity");
