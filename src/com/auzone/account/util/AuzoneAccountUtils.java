...
package com.auzone.account.util;
...
import com.auzone.account.auth.UpdateRequiredActivity;
...
public class AuzoneAccountUtils {

    private static final String TAG = AuzoneAccountUtils.class.getSimpleName();
    ...
    private AuzoneAccountUtils(){}

    public static void resetBackoff(SharedPreferences prefs) {
        if (AuzoneAccount.DEBUG) Log.d(TAG, "Resetting backoff");
        setBackoff(prefs, AuzoneAccount.DEFAULT_BACKOFF_MS);
    }

    private static int getBackoff(SharedPreferences prefs) {
        return prefs.getInt(AuzoneAccount.BACKOFF_MS, AuzoneAccount.DEFAULT_BACKOFF_MS);
    }

    private static void setBackoff(SharedPreferences prefs, int backoff) {
        SharedPreferences.Editor editor = prefs.edit();
        editor.putInt(AuzoneAccount.BACKOFF_MS, backoff);
        editor.commit();
    }
    ...
    public static void scheduleRetry(Context context, SharedPreferences prefs, Intent intent) {
        ...
        if (AuzoneAccount.DEBUG) Log.d(TAG, "Scheduling retry, backoff = " +
        ...
        am.set(AlarmManager.ELAPSED_REALTIME,
                ...
                retryPendingIntent);
        if (backoffTimeMs < AuzoneAccount.MAX_BACKOFF_MS) {
            setBackoff(prefs, backoffTimeMs * 2);
        }
    }

    public static void scheduleAuzoneAccountPing(Context context, Intent intent) {
        if (AuzoneAccount.DEBUG) Log.d(TAG, "Scheduling AuzoneAccount ping, starting = " +
    ...
    public static void cancelAuzoneAccountPing(Context context, Intent intent) {
        if (AuzoneAccount.DEBUG) Log.d(TAG, "Canceling AuzoneAccount ping");
        PendingIntent reRegisterPendingIntent = PendingIntent.getService(context, 0, intent, PendingIntent.FLAG_CANCEL_CURRENT);
        AlarmManager am = (AlarmManager) context.getSystemService(Context.ALARM_SERVICE);
        am.cancel(reRegisterPendingIntent);
    }

    public static Account getAuzoneAccountAccount(Context context) {
        final AccountManager am = AccountManager.get(context);
        Account[] accounts = am.getAccountsByType(AuzoneAccount.ACCOUNT_TYPE_AuzoneAccount);
    ...
    private static Intent getWifiSetupIntent(Context context) {
        ...
        intent.setComponent(
                new ComponentName(AuzoneAccount.WIFI_COMPONENT_PKG, AuzoneAccount.WIFI_COMPONENT_CLASS));
        intent.putExtra(AuzoneAccount.EXTRA_FIRST_RUN, true);
        intent.putExtra(AuzoneAccount.EXTRA_ALLOW_SKIP, false);
        intent.putExtra(AuzoneAccount.EXTRA_SHOW_BUTTON_BAR, true);
        intent.putExtra(AuzoneAccount.EXTRA_ONLY_ACCESS_POINTS, true);
        intent.putExtra(AuzoneAccount.EXTRA_AUTO_FINISH, true);
        return intent;
    }

    public static void launchWifiSetup(Activity context) {
        AuzoneAccountUtils.tryEnablingWifi(context);
        Intent intent = getWifiSetupIntent(context);
        context.startActivityForResult(intent, AuzoneAccount.REQUEST_CODE_SETUP_WIFI);
    }
    ...
    public static void launchWifiSetup(Fragment fragment) {
        ...
        AuzoneAccountUtils.tryEnablingWifi(context);
        Intent intent = getWifiSetupIntent(context);
        fragment.startActivityForResult(intent, AuzoneAccount.REQUEST_CODE_SETUP_WIFI);
    }
    ...
    public static String getUniqueDeviceId(Context context) {
        SharedPreferences prefs = context.getSharedPreferences(AuzoneAccount.SETTINGS_PREFERENCES, Context.MODE_PRIVATE);
                ...
                for (NetworkInterface networkInterface : interfaces) {
                    if (wifiInterface.equals(networkInterface.getDisplayName())) {
                        ...
                        if (mac != null) {
                            ...
                            if (AuzoneAccount.DEBUG) Log.d(TAG, "using wifi mac for id : " + buf.toString());
    ...
    public static String getDeviceSalt(AccountManager accountManager, Account account) {
        ...
        return accountManager.getUserData(account, AuzoneAccount.ACCOUNT_EXTRA_DEVICE_SALT);
    }

    public static String getDeviceSalt(Context context) {
        AccountManager accountManager = (AccountManager) context.getSystemService(Context.ACCOUNT_SERVICE);
        Account account = getAuzoneAccountAccount(context);
    ...
    public static byte[] getHmacSecret(AccountManager accountManager, Account account) {
        ...
        return Base64.decode(accountManager.getUserData(account,
                AuzoneAccount.ACCOUNT_EXTRA_HMAC_SECRET), Base64.NO_WRAP);
    }
    ...
    public static void setMinimumAppVersion(Context context, int minimumVersion) {
        SharedPreferences prefs = context.getSharedPreferences(AuzoneAccount.SETTINGS_PREFERENCES, Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = prefs.edit();
        editor.putInt(AuzoneAccount.MINIMUM_APP_VERSION, minimumVersion);
        editor.commit();
    }

    public static int getMinimumAppVersion(Context context) {
        SharedPreferences prefs = context.getSharedPreferences(AuzoneAccount.SETTINGS_PREFERENCES, Context.MODE_PRIVATE);
        return prefs.getInt(AuzoneAccount.MINIMUM_APP_VERSION, -1);
    }
    ...
    public static void showIncompatibleVersionNotification(Context context) {
        ...
        Notification notification = new Notification.Builder(context)
                .setContentTitle(context.getText(R.string.auzoneaccount_update_required_title))
                .setContentText(context.getText(R.string.auzoneaccount_update_required_short))
                .setSmallIcon(R.drawable.ic_dialog_alert)
                .setLargeIcon(((BitmapDrawable) context.getResources().getDrawable(R.drawable.icon)).getBitmap())
                .setContentIntent(contentIntent)
                .build();
        AuzoneAccountUtils.showNotification(context, AuzoneAccount.NOTIFICATION_ID_INCOMPATIBLE_VERSION, notification);
    }
}
