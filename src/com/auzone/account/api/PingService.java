...
public class PingService extends Service implements Response.ErrorListener, Response.Listener<PingResponse> {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (AuzoneAccountUtils.getAuzoneAccountAccount(context) == null) {
            AuzoneAccountUtils.cancelAuzoneAccountPing(context, intent);
            stopSelf();
        }
        mAuthClient = AuthClient.getInstance(context);
        boolean retry = intent.getBooleanExtra(EXTRA_RETRY, false);
        if (!retry) {
            AuzoneAccountUtils.resetBackoff(AuthClient.getInstance(context).getAuthPreferences());
        }
    ...
    public void onResponse(PingResponse pingResponse) {
        if (pingResponse.getStatusCode() == 200) {
            AuzoneAccountUtils.resetBackoff(mAuthClient.getAuthPreferences());
            final Context context = getApplicationContext();
            AuzoneAccountUtils.scheduleAuzoneAccountPing(context, getPingIntent(context));
    ...
    private void handleError() {
        ...
        AuzoneAccountUtils.scheduleRetry(getApplicationContext(), mAuthClient.getAuthPreferences(), intent);
        stopSelf();
    }
