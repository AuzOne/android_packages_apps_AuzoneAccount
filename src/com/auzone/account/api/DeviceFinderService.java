...
public class DeviceFinderService extends Service implements LocationListener,
    ...
    public static void reportLocation(Context context, Account account, final String keyId) {
        ...
        if (!sWakeLock.isHeld()) {
            if (AuzoneAccount.DEBUG) Log.v(TAG, "Acquiring wakelock");
            sWakeLock.acquire();
        }
    ...
    private void restartLocationUpdates() {
        mUpdateCount = 0;
        if (AuzoneAccount.DEBUG) Log.d(TAG, "Starting new LocationRequest");
    ...
    public void onDestroy() {
        super.onDestroy();
        if (sWakeLock != null) {
            if (AuzoneAccount.DEBUG) Log.v(TAG, "Releasing wakelock");
            sWakeLock.release();
        }
        mIsRunning = false;
    }
    ...
    private void onLocationChanged(final Location location, boolean fromLastLocation) {
        if (AuzoneAccount.DEBUG) Log.v(TAG, "onLocationChanged() " + location.toString());
    ...
    public void onResponse(Integer status) {
        if (AuzoneAccount.DEBUG) Log.v(TAG, "Successfully posted location");
    ...
    public void onErrorResponse(VolleyError volleyError) {
        if (volleyError.networkResponse != null) {
            ...
            if (AuzoneAccount.DEBUG) Log.v(TAG, "Location post error status = "+ statusCode);
        } else {
            if (AuzoneAccount.DEBUG) Log.v(TAG, "Location post error, no network response");
        }
    ...
    private void maybeStopLocationUpdates(float accuracy) {
        if (AuzoneAccount.DEBUG) Log.v(TAG, "Update count = "+ mUpdateCount);
    ...
    private void stopUpdates() {
        if (AuzoneAccount.DEBUG) Log.v(TAG, "Stopping location updates");
