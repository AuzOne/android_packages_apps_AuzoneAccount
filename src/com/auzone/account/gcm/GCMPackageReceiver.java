...
public class GCMPackageReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (!GCMUtil.googleServicesExist(context)) {
            if (AuzoneAccount.DEBUG) Log.d(TAG, "Google services not installed. skipping...");
            return;
        }
        final boolean needsUpgrade = GCMUtil.playServicesUpdateRequired(context);
        if (AuzoneAccount.DEBUG) Log.d(TAG, "Play Services needs upgrade = " + needsUpgrade);
        if (!needsUpgrade) {
            if (AuzoneAccountUtils.getAuzoneAccountAccount(context) != null) {
                GCMUtil.registerForGCM(context);
            }
            if (AuzoneAccount.DEBUG) Log.d(TAG, "Play Services upgraded, disabling receiver");
